...
/* ═══════════════════════════════════════════════════════════════════
   SNAKE DRAFT  (supports 1v1 / 2v2 / 3v3 / 4v4)
═══════════════════════════════════════════════════════════════════ */
...
function buildDE(teams,settings){
  ...
  propAll(M,F);
  return{matches:M,flow:F,bracketSize:bs,type:"DE"};
}
...
function buildSE(teams,settings){
  ...
  let M={},F={};
  if(bs<=4){
    M={M1:mk("M1","WB",1,"Semi Finals",s(0),s(3),f("Semi Finals")),
       M2:mk("M2","WB",1,"Semi Finals",s(1),s(2),f("Semi Finals")),
       M3:mk("M3","GF",2,"Grand Final",null,null,f("Grand Final"))};
    F={M1:{w:{id:"M3",s:"A"},l:null},M2:{w:{id:"M3",s:"B"},l:null},M3:{w:null,l:null}};
  } else {
    M={M1:mk("M1","WB",1,"Quarter Finals",s(0),s(7),f("Quarter Finals")),
       M2:mk("M2","WB",1,"Quarter Finals",s(3),s(4),f("Quarter Finals")),
       M3:mk("M3","WB",1,"Quarter Finals",s(1),s(6),f("Quarter Finals")),
       M4:mk("M4","WB",1,"Quarter Finals",s(2),s(5),f("Quarter Finals")),
       M5:mk("M5","WB",2,"Semi Finals",null,null,f("Semi Finals")),
       M6:mk("M6","WB",2,"Semi Finals",null,null,f("Semi Finals")),
       M7:mk("M7","GF",3,"Grand Final",null,null,f("Grand Final"))};
    F={M1:{w:{id:"M5",s:"A"},l:null},M2:{w:{id:"M5",s:"B"},l:null},
       M3:{w:{id:"M6",s:"A"},l:null},M4:{w:{id:"M6",s:"B"},l:null},
       M5:{w:{id:"M7",s:"A"},l:null},M6:{w:{id:"M7",s:"B"},l:null},M7:{w:null,l:null}};
  }
  propAll(M,F);
  return{matches:M,flow:F,bracketSize:bs,type:"SE"};
}
  ...
  return(
    <div style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:4}}>
      {options.map((o,i)=>{
        ...
        return(
          <button key={String(o)} onClick={()=>onChange(o)} style={{fontFamily:"Rajdhani,sans-serif",fontWeight:700,fontSize:".82rem",textTransform:"uppercase",padding:"6px 14px",borderRadius:7,border:`1px solid ${active?"var(--cyan)":"var(--border)"}`,background:active?"rgba(0,212,255,.12)":"transparent",color:active?"var(--cyan)":"var(--muted)",cursor:"pointer"}}>
...
function SettingSection({icon,title,children}){
  return(
    <div style={{borderBottom:"1px solid var(--border)",paddingBottom:16,marginBottom:16}}>
      <div style={{fontFamily:"Orbitron,sans-serif",fontSize:".72rem",fontWeight:700,letterSpacing:"1px",textTransform:"uppercase",color:"var(--cyan)",marginBottom:10}}>{icon} {title}</div>
      {children}
    </div>
  );
}
  ...
  return(
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.8)",backdropFilter:"blur(6px)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:600,padding:18}} onClick={e=>{if(e.target===e.currentTarget)onClose();}}>
      <div style={{background:"var(--card)",border:"1px solid var(--border)",borderRadius:16,padding:28,maxWidth:560,width:"100%",maxHeight:"90vh",overflowY:"auto",position:"relative",boxShadow:"0 24px 80px rgba(0,0,0,.6)"}}>
        ...
        <SettingSection icon="🎮" title="Game Mode">
          <ToggleGroup options={["1v1","2v2","3v3","4v4"]} value={settings.gameMode} onChange={v=>set("gameMode",v)}/>
          <div style={{fontSize:".78rem",color:"var(--muted)",marginTop:6}}>
            {settings.gameMode==="1v1"?"Each player competes solo — no team pairing.":`Players paired ${settings.gameMode} via snake draft.`}
          </div>
        </SettingSection>

        <SettingSection icon="🏆" title="Bracket Type">
          <ToggleGroup options={["DE","SE"]} labels={["Double Elimination","Single Elimination"]} value={settings.bracketType} onChange={v=>set("bracketType",v)}/>
          <div style={{fontSize:".78rem",color:"var(--muted)",marginTop:6}}>
            {settings.bracketType==="DE"?"Teams get a second chance in the Losers Bracket after one loss.":"One loss and you're out. Fast and brutal."}
          </div>
        </SettingSection>

        <SettingSection icon="👥" title="Max Teams">
          <ToggleGroup options={[4,8,16,32,0]} labels={["4","8","16","32","No limit"]} value={settings.maxTeams} onChange={v=>set("maxTeams",v)}/>
        </SettingSection>

        <SettingSection icon="📋" title="Early Round Format">
          <ToggleGroup options={["BO1","BO3","BO5","BO7"]} value={settings.earlyFormat} onChange={v=>set("earlyFormat",v)}/>
        </SettingSection>

        <SettingSection icon="🔥" title="Finals Format">
          <ToggleGroup options={["BO3","BO5","BO7"]} value={settings.finalsFormat} onChange={v=>set("finalsFormat",v)}/>
          <div style={{fontSize:".75rem",color:"var(--muted)",margin:"10px 0 6px",fontWeight:600,textTransform:"uppercase",letterSpacing:".4px"}}>Apply finals format from:</div>
          <ToggleGroup options={["SF","Final","GF"]} labels={["Semi-Finals","Finals only","Grand Final only"]} value={settings.finalsFrom} onChange={v=>set("finalsFrom",v)}/>
        </SettingSection>

        <SettingSection icon="⏱" title="Registration Deadline">
          <input type="datetime-local" value={settings.deadline} onChange={e=>set("deadline",e.target.value)}
            style={{background:"var(--surf)",border:"1px solid var(--border)",borderRadius:7,padding:"9px 13px",color:"var(--text)",fontFamily:"Rajdhani,sans-serif",fontSize:".9rem",width:"100%",colorScheme:"dark"}}/>
          {settings.deadline&&<div style={{fontSize:".8rem",color:"var(--muted)",marginTop:6}}>Sign-ups lock: <strong style={{color:"var(--text)"}}>{new Date(settings.deadline).toLocaleString()}</strong></div>}
          {settings.deadline&&<button onClick={()=>set("deadline","")} style={{marginTop:6,fontFamily:"Rajdhani,sans-serif",fontWeight:700,fontSize:".78rem",textTransform:"uppercase",padding:"5px 12px",borderRadius:6,border:"1px solid var(--border)",background:"transparent",color:"var(--muted)",cursor:"pointer"}}>Clear deadline</button>}
        </SettingSection>

        <SettingSection icon="⚡" title="Overtime Rules">
          <ToggleGroup options={[true,false]} labels={["Golden Goal (sudden death)","No overtime"]} value={settings.goldenGoal} onChange={v=>set("goldenGoal",v)}/>
        </SettingSection>

        <button onClick={onClose} style={{fontFamily:"Rajdhani,sans-serif",fontWeight:800,fontSize:".9rem",textTransform:"uppercase",letterSpacing:".5px",padding:"13px 18px",borderRadius:8,border:"none",cursor:"pointer",background:"linear-gradient(135deg,var(--cyan),#008ab8)",color:"#000",width:"100%",marginTop:4}}>
          Save Settings ✓
        </button>
      </div>
    </div>
...
function Countdown({deadline}){
  ...
  const closed=rem<=0;
  return(
    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",background:closed?"rgba(255,71,87,.08)":"rgba(0,212,255,.08)",border:`1px solid ${closed?"rgba(255,71,87,.3)":"rgba(0,212,255,.3)"}`,borderRadius:9,padding:"10px 16px",marginBottom:14}}>
      <span style={{fontSize:".82rem",fontWeight:600,color:closed?"var(--red)":"var(--cyan)"}}>{closed?"🔒 Registration Closed":"⏱ Sign-ups close in"}</span>
      {!closed&&<span style={{fontFamily:"Orbitron,sans-serif",fontSize:"1.1rem",fontWeight:900,color:"var(--gold)"}}>{formatCountdown(rem)}</span>}
    </div>
  );
}

function TeamCard({t,onRename}){
        ...
        <div style={{display:"flex",alignItems:"center",gap:6,flex:1,minWidth:0}}>
          {editing?(
            <div style={{display:"flex",alignItems:"center",gap:5,flex:1,minWidth:0}}>
              <input ref={ref} value={draft} maxLength={30} onChange={e=>setDraft(e.target.value)}
                onKeyDown={e=>{if(e.key==="Enter")save();if(e.key==="Escape"){setDraft(t.name);setEditing(false);}}}
                onBlur={save}
                style={{background:"var(--surf)",border:"1px solid var(--cyan)",borderRadius:6,padding:"4px 8px",color:"var(--text)",fontFamily:"Orbitron,sans-serif",fontSize:".78rem",fontWeight:700,width:"100%",outline:"none"}}/>
            ...
            <>
              ...
              <button onClick={()=>{setDraft(t.name);setEditing(true);}} style={{background:"none",border:"none",cursor:"pointer",color:"var(--muted)",fontSize:".9rem",padding:"2px 4px",borderRadius:4,flexShrink:0}}>✏️</button>
            </>
          )}
        </div>
      </div>
      <div style={{display:"flex",flexDirection:"column",gap:6,marginBottom:12}}>
        {t.players.map(p=>(
          <div key={p.id} style={{display:"flex",alignItems:"center",gap:7}}>
            <span style={{fontWeight:700,fontSize:".7rem",minWidth:85,color:rankColor(p.rank)}}>{p.rank}</span>
...
function MatchCard({match:m,onClick,isGF}){
  const ready=m.teamA&&m.teamB&&!m.isComplete&&!m.isBye;
  const borderColor=m.isComplete?"rgba(0,255,136,.18)":ready?"rgba(0,212,255,.4)":m.isBye?"var(--border)":isGF?"rgba(255,215,0,.45)":"var(--border)";
  return(
...
function SlotRow({team,winner,score,bye}){
  ...
  return(
    <div style={{display:"flex",alignItems:"center",gap:6,padding:"3px 0",fontWeight:600,...(isW?{color:"var(--green)"}:{})}}>
      <span style={{fontFamily:"Orbitron,sans-serif",fontSize:".58rem",color:"var(--muted)",minWidth:20}}>#{team.seed}</span>
      <span style={{flex:1,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis",fontSize:".82rem"}}>{team.name}</span>
      {score!=null&&<span style={{fontFamily:"Orbitron,sans-serif",fontWeight:900,fontSize:".95rem",color:"var(--cyan)"}}>{score}</span>}
    </div>
  );
}

function BracketSection({title,color,rounds,openModal}){
      ...
      <div style={{display:"flex",gap:12,overflowX:"auto",paddingBottom:5}}>
        {Object.entries(rounds).map(([label,matches])=>(
          <div key={label} style={{minWidth:180,display:"flex",flexDirection:"column",gap:9}}>
            ...
            {matches.map(m=><MatchCard key={m.id} match={m} onClick={()=>openModal(m)}/>)}
          </div>
        ))}
      </div>
    </div>
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.75)",backdropFilter:"blur(6px)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:500,padding:18}} onClick={e=>{if(e.target===e.currentTarget)onClose();}}>
      <div style={{background:"var(--card)",border:"1px solid var(--border)",borderRadius:16,padding:28,maxWidth:460,width:"100%",position:"relative",boxShadow:"0 24px 80px rgba(0,0,0,.5)"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr auto 1fr",gap:14,alignItems:"center",marginBottom:10}}>
          <div style={{display:"flex",flexDirection:"column",gap:9,textAlign:"center"}}>
            <div style={{fontWeight:700,fontSize:".9rem"}}>#{m.teamA?.seed} {m.teamA?.name}</div>
            <input style={{background:"var(--surf)",border:"2px solid var(--border)",borderRadius:10,padding:"12px 8px",color:"var(--text)",fontFamily:"Orbitron,sans-serif",fontSize:"2rem",fontWeight:900,textAlign:"center",width:"100%"}} type="number" min="0" max={max} placeholder="0" value={sA} onChange={e=>setSA(e.target.value)}/>
          </div>
          <div style={{fontFamily:"Orbitron,sans-serif",fontSize:".85rem",color:"var(--orange)",textAlign:"center",fontWeight:700}}>VS</div>
          <div style={{display:"flex",flexDirection:"column",gap:9,textAlign:"center"}}>
            <div style={{fontWeight:700,fontSize:".9rem"}}>#{m.teamB?.seed} {m.teamB?.name}</div>
            <input style={{background:"var(--surf)",border:"2px solid var(--border)",borderRadius:10,padding:"12px 8px",color:"var(--text)",fontFamily:"Orbitron,sans-serif",fontSize:"2rem",fontWeight:900,textAlign:"center",width:"100%"}} type="number" min="0" max={max} placeholder="0" value={sB} onChange={e=>setSB(e.target.value)}/>
          </div>
        </div>
  ...
  return(
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.8)",backdropFilter:"blur(6px)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:500,padding:18}}>
      <div style={{background:"var(--card)",border:"1px solid var(--border)",borderRadius:16,padding:28,maxWidth:440,width:"100%",textAlign:"center",boxShadow:"0 24px 80px rgba(0,0,0,.5)"}}>
        ...
        <p style={{color:"var(--muted)",lineHeight:1.65,marginBottom:10,fontSize:".9rem"}}>The Losers Bracket champion just won the Grand Final! One more deciding game determines the ultimate champion.</p>
...
export default function App() {
  ...
  const[resetWinner,setResetW]    =useState(null);
  ...
  const[spectCode]                =useState("RL-"+Math.random().toString(36).substr(2,6).toUpperCase());
  ...
  function addPlayer(){
    if(!form.name.trim()||signupsLocked)return;
  ...
  function loadDemo(){setPlayers(DEMO_PLAYERS.map(p=>({...p,seed:rankSeed(p.rank)})));}
  ...
  function openModal(m){if(m.isBye||m.isComplete||!m.teamA||!m.teamB)return;setModal(m);setSA("");setSB("");}
  function submitScore(){
    ...
    const all=!Object.values(nb.matches).some(m=>!m.isComplete&&!m.isBye&&m.teamA&&m.teamB);
    if(all){setPhase("results");return;}
  ...
  return(
    <>
      ...
      <div style={{minHeight:"100vh",background:"var(--bg)",backgroundImage:"radial-gradient(ellipse 80% 50% at 10% -10%,rgba(0,212,255,.07),transparent),radial-gradient(ellipse 60% 60% at 90% 110%,rgba(168,85,247,.07),transparent)"}}>
        {/* NAV */}
        <nav style={{position:"sticky",top:0,zIndex:200,display:"flex",alignItems:"center",justifyContent:"space-between",padding:"10px 20px",background:"rgba(5,8,15,.9)",borderBottom:"1px solid var(--border)",backdropFilter:"blur(12px)"}}>
          <div style={{fontFamily:"Orbitron,sans-serif",fontSize:".95rem",fontWeight:900,color:"var(--cyan)",letterSpacing:1,display:"flex",alignItems:"center",gap:7}}>
            🚀 <span>RL <span style={{fontWeight:400,color:"rgba(0,212,255,.55)"}}>Tournament</span></span>
          </div>
          <div style={{display:"flex",alignItems:"center",gap:14}}>
            <div style={{display:"flex",alignItems:"center",gap:16}}>
              {["Sign-Ups","Teams","Bracket","Results"].map((label,i)=>{
                const p=phaseNames[i],idx=phaseNames.indexOf(phase);
                const active=phase===p,done=idx>i;
                return(
                  <div key={p} style={{display:"flex",alignItems:"center",gap:6}}>
                    <div style={{width:8,height:8,borderRadius:"50%",background:active?"var(--cyan)":done?"var(--green)":"var(--muted)",boxShadow:active?"0 0 10px var(--cyan)":"none",transition:"all .3s"}}/>
                    <span style={{fontSize:".65rem",fontWeight:700,textTransform:"uppercase",letterSpacing:".6px",color:active?"var(--cyan)":done?"var(--green)":"var(--muted)"}}>{label}</span>
                  </div>
                );
              })}
            </div>
            <div style={{display:"flex",alignItems:"center",gap:8}}>
              <button onClick={()=>setShowFAQ(true)} style={{fontFamily:"Orbitron,sans-serif",fontSize:".65rem",fontWeight:700,letterSpacing:"1px",textTransform:"uppercase",padding:"6px 12px",borderRadius:7,border:"1px solid rgba(0,212,255,.3)",background:"rgba(0,212,255,.07)",color:"var(--cyan)",cursor:"pointer"}}>
                ❓ FAQ
              </button>
              {(phase==="signup"||phase==="teams")&&(
                <button onClick={()=>setShowSettings(true)} style={{fontFamily:"Orbitron,sans-serif",fontSize:".65rem",fontWeight:700,letterSpacing:"1px",textTransform:"uppercase",padding:"6px 12px",borderRadius:7,border:"1px solid rgba(168,85,247,.4)",background:"rgba(168,85,247,.08)",color:"var(--purple)",cursor:"pointer"}}>
                  ⚙️ Settings
                </button>
              )}
            </div>
        ...
        <div style={{maxWidth:1200,margin:"0 auto",padding:"22px 18px 60px"}}>
          {phase==="signup"&&<SignupView players={players} form={form} setForm={setForm} addPlayer={addPlayer} loadDemo={loadDemo} removePlayer={id=>setPlayers(p=>p.filter(x=>x.id!==id))} closeSignups={closeSignups} settings={settings} signupsLocked={signupsLocked}/>}
          {phase==="teams"&&<TeamsView teams={teams} wildcards={wildcards} startTournament={startTournament} back={()=>setPhase("signup")} rename={renameTeam} settings={settings}/>}
          {phase==="bracket"&&<BracketPhase groups={groups} featured={featured} spectCode={spectCode} currentRound={currentRound} upsets={upsets} openModal={openModal} bracket={bracket} settings={settings}/>}
        ...
        {modal&&<ScoreModal match={modal} sA={sA} sB={sB} setSA={setSA} setSB={setSB} onSubmit={submitScore} onClose={()=>setModal(null)}/>}
        {showReset&&gfId&&<ResetModal gf={bracket.matches[gfId]} onPick={handleReset}/>}
...
function SignupView({players,form,setForm,addPlayer,loadDemo,removePlayer,closeSignups,settings,signupsLocked}){
  ...
  return(
    <div>
      ...
      {signupsLocked&&<div style={{background:"rgba(255,71,87,.1)",border:"1px solid rgba(255,71,87,.3)",borderRadius:9,padding:"12px 16px",marginBottom:14,color:"var(--red)",fontWeight:600,fontSize:".9rem"}}>🔒 Registration is closed. No new players can be added.</div>}
      <div style={{display:"grid",gridTemplateColumns:"300px 1fr",gap:16}}>
        <div style={{...C.card,display:"flex",flexDirection:"column",gap:9}}>
          ...
          <select style={{...C.inp,...(signupsLocked?{opacity:.4,pointerEvents:"none"}:{})}} value={form.rank} onChange={e=>setForm(f=>({...f,rank:e.target.value}))} disabled={signupsLocked}>
          ...
          <button style={C.ghostBtn} onClick={loadDemo} disabled={signupsLocked}>🎮 Load 8 Demo Players</button>
        </div>
        <div style={C.card}>
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:12}}>
          ...
          {!players.length&&<p style={{color:"var(--muted)",fontStyle:"italic",fontSize:".85rem",padding:"16px 0",textAlign:"center"}}>No players yet. Use the form to register!</p>}
          <div style={{maxHeight:280,overflowY:"auto",display:"flex",flexDirection:"column",gap:4,marginBottom:10}}>
            {sorted.map((p,i)=>(
              <div key={p.id} style={C.prow}>
                ...
                <button style={{background:"none",border:"none",color:"var(--muted)",cursor:"pointer",fontSize:".85rem",padding:"1px 5px",borderRadius:4}} onClick={()=>removePlayer(p.id)}>✕</button>
              </div>
            ))}
          </div>
...
function TeamsView({teams,wildcards,startTournament,back,rename,settings}){
  return(
    <div>
      <h1 style={C.h1}>⚡ Team Reveal</h1>
      <p style={C.sub}>{settings.gameMode==="1v1"?"1v1 — each player competes solo. Click ✏️ to rename.":`Balanced via snake draft (${settings.gameMode}). Click ✏️ on any card to rename.`}</p>
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(220px,1fr))",gap:14,marginBottom:16}}>
        {teams.map(t=><TeamCard key={t.id} t={t} onRename={rename}/>)}
      </div>
      {wildcards.length>0&&<div style={{...C.card,color:"var(--orange)",borderColor:"rgba(255,107,53,.25)",marginBottom:16,fontSize:".88rem"}}>🃏 <strong>Wildcard Sub{wildcards.length>1?"s":""}:</strong> {wildcards.map(p=><span key={p.id}>{p.name} <span style={{color:rankColor(p.rank)}}>({p.rank})</span> </span>)} — step in if any player drops out</div>}
...
function BracketPhase({groups,featured,spectCode,currentRound,upsets,openModal,bracket,settings}){
  return(
    <div>
      <h1 style={C.h1}>🏟️ Live Bracket</h1>
      {currentRound?<p style={C.sub}>Round {currentRound} in progress · {settings.goldenGoal?"Golden goal OT":"No overtime"} · Click any ready match to enter scores</p>:<p style={C.sub}>All matches complete!</p>}
      {featured&&(
      ...
      <div style={{display:"flex",flexDirection:"column",gap:14,marginBottom:18}}>
        <BracketSection title="🏆 Winners Bracket" color="wb" rounds={groups.WB} openModal={openModal}/>
        {bracket?.type==="DE"&&Object.keys(groups.LB).length>0&&<BracketSection title="💀 Losers Bracket" color="lb" rounds={groups.LB} openModal={openModal}/>}
        {groups.GF?.length>0&&(
          <div style={{...C.card,borderColor:"rgba(255,215,0,.3)"}}>
            ...
            <div style={{display:"flex",gap:12}}><div style={{minWidth:180,display:"flex",flexDirection:"column",gap:9}}>{groups.GF.map(m=><MatchCard key={m.id} match={m} onClick={()=>openModal(m)} isGF/>)}</div></div>
          </div>
        )}
      </div>
  ...
  return(
    <div>
      ...
      <div style={{display:"flex",justifyContent:"center",alignItems:"flex-end",gap:10,marginBottom:24,flexWrap:"wrap"}}>
        {standings.second&&(
          <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:7,textAlign:"center",minWidth:145}}>
            <div style={{fontSize:"2rem"}}>🥈</div>
            <div style={{fontFamily:"Orbitron,sans-serif",fontSize:".88rem",fontWeight:700}}>{standings.second.name}</div>
            <div style={{fontSize:".76rem",color:"var(--muted)",marginBottom:3}}>{standings.second.players.map(p=>p.name).join(" & ")}</div>
            <div style={podiumStyle(58,"rgba(192,192,192,.25)","rgba(192,192,192,.25)")}/>
          </div>
        )}
        <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:7,textAlign:"center",minWidth:145}}>
          <div style={{fontSize:"2rem"}}>🏆</div>
          <div style={{fontFamily:"Orbitron,sans-serif",fontSize:".88rem",fontWeight:700,color:"var(--gold)"}}>{standings.first?.name}</div>
          <div style={{fontSize:".76rem",color:"var(--muted)",marginBottom:3}}>{standings.first?.players.map(p=>p.name).join(" & ")}</div>
          <div style={podiumStyle(80,"rgba(255,215,0,.35)","rgba(255,215,0,.4)")}/>
        </div>
        {standings.third&&(
          <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:7,textAlign:"center",minWidth:145}}>
            <div style={{fontSize:"2rem"}}>🥉</div>
            <div style={{fontFamily:"Orbitron,sans-serif",fontSize:".88rem",fontWeight:700}}>{standings.third.name}</div>
            <div style={{fontSize:".76rem",color:"var(--muted)",marginBottom:3}}>{standings.third.players.map(p=>p.name).join(" & ")}</div>
            <div style={podiumStyle(42,"rgba(205,127,50,.2)","rgba(205,127,50,.25)")}/>
          </div>
        )}
      </div>
      ...
      <div style={C.card}>
        ...
        <table style={{width:"100%",borderCollapse:"collapse",fontSize:".84rem"}}>
          ...
          <tbody>
            {teams.map(t=>{
              const pl=t.id===standings.first?.id?1:t.id===standings.second?.id?2:t.id===standings.third?.id?3:null;
              return(
                <tr key={t.id} style={pl===1?{background:"rgba(255,215,0,.04)"}:pl===2?{background:"rgba(192,192,192,.03)"}:pl===3?{background:"rgba(205,127,50,.03)"}:{}}>
                  <td style={{padding:"9px 9px 9px 0",borderBottom:"1px solid rgba(255,255,255,.04)"}}>{pl===1?"🏆":pl===2?"🥈":pl===3?"🥉":""}#{t.seed}</td>
                  <td style={{padding:"9px 9px 9px 0",borderBottom:"1px solid rgba(255,255,255,.04)"}}><strong>{t.name}</strong></td>
                  <td style={{padding:"9px 9px 9px 0",borderBottom:"1px solid rgba(255,255,255,.04)"}}>{t.players.map(p=><span key={p.id} style={{display:"inline-flex",alignItems:"center",gap:4,marginRight:9,fontSize:".78rem"}}><span style={{color:rankColor(p.rank)}}>{p.rank}</span> {p.name}</span>)}</td>
                </tr>
              );
            })}
          </tbody>
...
function SupportButton({onClick}){
  return(
    <button onClick={onClick} style={{
      ...
      fontFamily:"Orbitron,sans-serif",fontWeight:700,fontSize:".68rem",
      letterSpacing:"1px",textTransform:"uppercase",
      padding:"10px 16px",borderRadius:30,border:"1px solid rgba(145,70,255,.5)",
      background:"rgba(145,70,255,.15)",color:"#bf94ff",
      cursor:"pointer",backdropFilter:"blur(8px)",
      boxShadow:"0 4px 20px rgba(145,70,255,.25)",
      transition:"all .2s"
    }}
    onMouseEnter={e=>{e.currentTarget.style.background="rgba(145,70,255,.28)";e.currentTarget.style.boxShadow="0 4px 28px rgba(145,70,255,.45)";}}
    onMouseLeave={e=>{e.currentTarget.style.background="rgba(145,70,255,.15)";e.currentTarget.style.boxShadow="0 4px 20px rgba(145,70,255,.25)";}}>
      💜 Support
    </button>
  );
}
...
function SupportModal({onClose}){
  return(
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.7)",backdropFilter:"blur(6px)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:600,padding:18}}
      ...
      <div style={{background:"var(--card)",border:"1px solid rgba(145,70,255,.3)",borderRadius:16,padding:32,maxWidth:400,width:"100%",position:"relative",boxShadow:"0 24px 80px rgba(0,0,0,.6)",textAlign:"center"}}>
        ...
        <p style={{color:"var(--muted)",fontSize:".88rem",lineHeight:1.7,marginBottom:24}}>
          This tournament tool is built and maintained by <strong style={{color:"#bf94ff"}}>chilly7383</strong>. 
          If you enjoy it, consider showing some support — it means a lot and helps keep the project going!
        </p>

        <div style={{display:"flex",flexDirection:"column",gap:10}}>
          <a href="https://www.twitch.tv/chilly7383" target="_blank" rel="noopener noreferrer"
            style={{display:"flex",alignItems:"center",justifyContent:"center",gap:10,padding:"12px 20px",borderRadius:10,background:"rgba(145,70,255,.15)",border:"1px solid rgba(145,70,255,.4)",color:"#bf94ff",fontFamily:"Orbitron,sans-serif",fontWeight:700,fontSize:".8rem",letterSpacing:"1px",textTransform:"uppercase",textDecoration:"none",transition:"all .18s"}}
            onMouseEnter={e=>e.currentTarget.style.background="rgba(145,70,255,.28)"}
            onMouseLeave={e=>e.currentTarget.style.background="rgba(145,70,255,.15)"}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:10,padding:"12px 20px",borderRadius:10,background:"rgba(255,255,255,.04)",border:"1px solid var(--border)",color:"var(--muted)",fontFamily:"Orbitron,sans-serif",fontWeight:700,fontSize:".75rem",letterSpacing:"1px",textTransform:"uppercase",justifyContent:"center"}}>
            💰 Donate link coming soon
          </div>
        </div>

        <p style={{color:"var(--muted)",fontSize:".75rem",marginTop:20,lineHeight:1.6}}>
          Even just watching the stream and hanging out in chat makes a difference. Thank you! 🚀
        </p>
      </div>
    </div>
  );
...
function FAQModal({onClose}){
  ...
  const faqs=[
    {q:"How do I register?",
     a:"Contact the host via stream chat or DM with your Rocket League gamertag and current rank. The host adds you before sign-ups close. You can't register yourself — the host controls the roster."},
    {q:"How does team balancing work?",
     a:"Players are sorted by rank, then paired using a snake draft. The highest-ranked player gets paired with the lowest-ranked, the second-highest with the second-lowest, and so on. This gives every team a roughly equal combined skill level — no stacked teams."},
    {q:"What is double elimination?",
     a:"You need to lose twice to be knocked out. After your first loss, you drop into the Losers Bracket and get a second chance. Win enough Losers Bracket matches and you can still reach the Grand Final — you just have no more margin for error."},
    {q:"What is single elimination?",
     a:"One loss and you're out immediately. Every match is a must-win. It's faster than double elimination and better suited for larger tournaments or tight time schedules."},
    {q:"How do I spectate a match?",
     a:'In Rocket League, go to Play → Custom Games → Join Private Match and enter the spectator code shown on stream. You MUST join before the match starts — once the game begins, the lobby is locked and you cannot join mid-game.'},
    {q:"What happens if my team no-shows?",
     a:"If your team doesn't join the lobby within 5 minutes of the round starting, your opponent is awarded a walkover win automatically. Make sure you're online, in a party, and ready before each round begins."},
    {q:"What is a bracket reset?",
     a:"In double elimination, the Grand Final is between the Winners Bracket champion (0 losses) and the Losers Bracket champion (1 loss). If the LB champion wins game 1, both players now have exactly 1 loss — so a reset match is played to find the true winner."},
    {q:"What format are the matches played in?",
     a:"The host sets the format in Settings before the tournament starts. Common formats are Best of 3 (first to win 2 games) for early rounds and Best of 5 (first to win 3) for finals. Check the lobby schedule on screen to see the format for your match."},
    {q:"What happens in overtime?",
     a:"If golden goal is enabled (the default), the first goal scored in overtime wins the game immediately — no extra time, sudden death. If the host has disabled golden goal, overtime works like a standard extra period."},
    {q:"Can I use any car or item?",
     a:"Yes — there are no restrictions on cars, decals, or items. Use whatever you're comfortable with. Competitive settings apply (no ball cam restrictions, etc.) unless the host specifies otherwise."},
  ];

  const steps=[
    {n:1,title:"Configure your settings first",
     body:"Before opening sign-ups, click ⚙️ Settings and set your Game Mode (1v1/2v2/3v3/4v4), Bracket Type (Double or Single Elimination), Match Format (BO3/BO5), and Max Teams. If you know when sign-ups will close, set a Registration Deadline — the form locks automatically when it expires."},
    {n:2,title:"Open sign-ups and collect registrations",
     body:"Share the sign-up method with your community — this could be a stream command (!register), a Discord form, or just chat. For each player, enter their gamertag and current rank in the Sign-Ups tab. Players are automatically sorted by rank so you can see balance at a glance."},
    {n:3,title:"Close sign-ups and review teams",
     body:"Click 'Close Sign-Ups & Generate Teams'. The app pairs players automatically via snake draft. Review the Teams page — if you want to rename any team to something more fun, click the ✏️ button on the card. Once you're happy, click 'Generate Bracket & Start Tournament'."},
    {n:4,title:"Set up private lobbies in Rocket League",
     body:"For each match in a round, one player creates a Private Match lobby. Go to Play → Custom Games → Create Private Match. Set the password to something simple (e.g. 'lobby1'). Share the lobby name and password with both teams via Discord DM or stream overlay. For the Featured Match, also share a spectator code so viewers can watch."},
    {n:5,title:"Announce the featured match",
     body:"Before each round, the app picks a Featured Match (closest seed matchup = most exciting). Share the spectator code at least 3 minutes before the round starts so viewers can join. Remind spectators they MUST join before the match begins — Rocket League locks the lobby once the game starts."},
    {n:6,title:"Run the round simultaneously",
     body:"All matches in a round happen at the same time in parallel lobbies. Once all lobbies are set up and both teams are ready, call 'Go!' for all lobbies at once. The app's lobby schedule shows Lobby 1, Lobby 2, etc. — assign each to a match so there's no confusion."},
    {n:7,title:"Enter scores as results come in",
     body:"When a match finishes, click on that match card in the bracket and enter the final series score (e.g. 2-1 for a BO3). The bracket updates instantly — winners advance, losers drop to the Losers Bracket (if DE). The app will flag any upsets automatically."},
    {n:8,title:"Handle no-shows and drop-outs",
     body:"If a team doesn't show within 5 minutes, enter the score as a walkover (2-0 or 3-0 for the opponent). If a player drops out mid-tournament and you have a wildcard sub, substitute them before the next round — rename the team card on the Teams page to reflect the change."},
    {n:9,title:"Between rounds",
     body:"After all matches in a round are complete, the app generates the next round automatically. Announce the next Featured Match with its spectator code, give teams 10 minutes to set up their lobbies, and call 'Go!' again. Repeat until the Grand Final."},
    {n:10,title:"Grand Final and bracket reset",
     body:"The Grand Final is WB champion vs LB champion. If the LB champion wins, a bracket reset match is required (since both now have 1 loss). The app will prompt you — just pick who won the reset. The Results page shows the full podium, MVP, and biggest upset of the tournament."},
  ];

  const btnStyle=(active)=>({fontFamily:"Orbitron,sans-serif",fontWeight:700,fontSize:".7rem",letterSpacing:"1px",textTransform:"uppercase",padding:"8px 18px",borderRadius:7,border:`1px solid ${active?"var(--cyan)":"var(--border)"}`,background:active?"rgba(0,212,255,.12)":"transparent",color:active?"var(--cyan)":"var(--muted)",cursor:"pointer",transition:"all .18s"});
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.8)",backdropFilter:"blur(6px)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:600,padding:18}} onClick={e=>{if(e.target===e.currentTarget)onClose();}}>
      <div style={{background:"var(--card)",border:"1px solid var(--border)",borderRadius:16,padding:28,maxWidth:680,width:"100%",maxHeight:"88vh",display:"flex",flexDirection:"column",position:"relative",boxShadow:"0 24px 80px rgba(0,0,0,.6)"}}>
        ...
        <div style={{overflowY:"auto",flex:1,paddingRight:4}}>
          {tab==="faq"&&(
            <div style={{display:"flex",flexDirection:"column",gap:10}}>
              <p style={{fontSize:".82rem",color:"var(--muted)",marginBottom:4,lineHeight:1.6}}>Common questions from players and viewers. Share these answers with your community or add them as Nightbot commands.</p>
              {faqs.map((item,i)=>(
          ...
          {tab==="host"&&(
            <div style={{display:"flex",flexDirection:"column",gap:12}}>
              <p style={{fontSize:".82rem",color:"var(--muted)",marginBottom:4,lineHeight:1.6}}>A step-by-step walkthrough for first-time tournament hosts. Follow these in order before and during your stream.</p>
              <div style={{background:"rgba(255,215,0,.06)",border:"1px solid rgba(255,215,0,.2)",borderRadius:8,padding:"10px 14px",fontSize:".82rem",color:"var(--gold)",marginBottom:4,lineHeight:1.6}}>
                💡 <strong>Before you go live:</strong> run through steps 1–3 so everything is set up before your audience joins.
              </div>
              {steps.map(s=>(
                <StepItem key={s.n} n={s.n} title={s.title} body={s.body}/>
              ))}
            </div>
          )}
        </div>
...
function FAQItem({q,a}){
  ...
  return(
    <div style={{border:"1px solid var(--border)",borderRadius:9,overflow:"hidden",transition:"border-color .2s",...(open?{borderColor:"rgba(0,212,255,.3)"}:{})}}>
      <button onClick={()=>setOpen(o=>!o)} style={{width:"100%",display:"flex",alignItems:"center",justifyContent:"space-between",gap:10,padding:"11px 14px",background:"none",border:"none",cursor:"pointer",textAlign:"left"}}>
        <span style={{fontWeight:700,fontSize:".88rem",color:"var(--text)"}}>{q}</span>
        <span style={{color:"var(--cyan)",fontSize:"1rem",flexShrink:0,transform:open?"rotate(180deg)":"rotate(0)",transition:"transform .2s"}}>▾</span>
      </button>
      {open&&<div style={{padding:"0 14px 12px",fontSize:".85rem",color:"var(--muted)",lineHeight:1.7,borderTop:"1px solid var(--border)"}}><div style={{paddingTop:10}}>{a}</div></div>}
    </div>
  );
}
...
function StepItem({n,title,body}){
  return(
    <div style={{display:"flex",gap:14,padding:"14px",background:"var(--surf)",borderRadius:10,border:"1px solid var(--border)"}}>
      <div style={{fontFamily:"Orbitron,sans-serif",fontSize:".9rem",fontWeight:900,color:"var(--cyan)",minWidth:28,paddingTop:2}}>{String(n).padStart(2,"0")}</div>
      <div>
        <div style={{fontWeight:700,fontSize:".9rem",marginBottom:5}}>{title}</div>
        <div style={{fontSize:".82rem",color:"var(--muted)",lineHeight:1.7}}>{body}</div>
      </div>
    </div>
  );
}
